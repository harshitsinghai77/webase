...
function InterfaceBody(props) {
    ...
    const setImageUrlFunc = (url) => {
      setImageUrl(url)
      setSpinner(true)
      window.axiosInstance.post(`http://10.12.14.60:5000/${getFlaskURL[modelId]}`, {imageUrl: url})
        .then((res) => {
          setSpinner(false)
    ...
    const customUpload = async ({ onError, onSuccess, file }) => {
        ...
        try {
          ...
          storage.ref('WeBase').child(fileName).getDownloadURL().then(url => {
            const dataObj = {
              userID: userID,
            ...
            window.axiosInstance.post('model/upload-image', dataObj)
              .then((data) => {
    ...
    const ImageHolder = ({data}) => <div className={styles.Test_previewContainer__3ZS6L}> <img src={`data:image/jpg;base64,${data}`} className={styles.Test_preview__3AvJR} /> </div>
    const ImageHolder2 = ({data}) => <div className={styles.Test_previewContainer__3ZS6L}> <img onError={null} src={data} className={styles.Test_preview__3AvJR} /> </div>
    
    return (
      <div className= {styles.Test_grid2__3nZyM}>
          <div className={styles.Test_leftCol__1XSvT}>
            <div style={{overflow: 'auto', display: 'grid', gridTemplateRows: 'auto 1fr', height: '100%', background: 'rgb(249, 249, 249)'}}>
                <div className={styles.Analyze_imageGridRoot__XgStN} style={{filter: 'blur(0px)'}}>
                    <div className={styles.Analyze_imageGrid__1_hgM}>
                        <ImageGrid modelId = {modelId} userID = {userID} setImageUrlFromChild = {setImageUrlFunc} newUploadImage = {newUploadImage} />
                    </div>
                </div>
            </div>   

            <Upload style = {{padding: '100px'}} showUploadList={false} {...imageProperties} beforeUpload={beforeUpload} customRequest={customUpload}>
          ...
          <div className = {styles.Test_rightCol__2W8HC}>
              ...
              <div style={{borderRadius: '6px', border: '1px solid rgb(222, 222, 222)', overflowY: 'auto', overflowX: 'hidden', display: 'grid; grid-template-rows: auto 1fr', paddingTop: '8px'}}>
                  ...
                  <div style={{position: 'relative', padding: '0px 24px'}}>
                    <pre style={{margin: '0px'}}>
                        {spinner && <Spin spinning = {spinner} /> || isBase64(jsonResponse) && <ImageHolder data = {jsonResponse} /> || checkString(jsonResponse) && <ImageHolder2 data = {jsonResponse} /> || JSON.stringify(jsonResponse, null, 2) }
                    </pre>
                  </div>
              </div>
