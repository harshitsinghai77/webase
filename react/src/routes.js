...
function isLoggedIn(WrapperComponent, url) {
  return (props) => {
    if (!getToken()  || !getCurrentUser()) return <Redirect noThrow to={url || '/login'} />;
...
function checkedLogedIn(WrapperComponent) {
  return (props) => {
    if (getToken() && getCurrentUser()) return <Redirect noThrow to={`/create-new-project/${getCurrentUser()}`} />;
    return <WrapperComponent {...props} />;
  };
}
...
function withDashboard(WrapperComponent) {
  return props => (
    <div className={styles.container}>
      <SideBar getCurrentUser = {getCurrentUser()} Component = {<WrapperComponent  {...props} />} />
    </div>
  );
}
